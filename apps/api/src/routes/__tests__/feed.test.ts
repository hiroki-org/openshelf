...
describe("feed routes", () => {
    ...
    it("GET /feed/users/:id/atom.xml returns 404 when user is not found", async () => {
        mockDb.select = vi.fn(() => makeQuery({ getResult: null }));

        const app = await createTestApp();
        const env = createTestEnv();

        const res = await app.request("http://localhost/feed/users/no-such-user/atom.xml", {}, env as any);

        expect(res.status).toBe(404);
        await expect(res.json()).resolves.toMatchObject({ error: "User not found" });
    });

    it("GET /feed/users/:id/atom.xml uses user name as title fallback when displayName is null", async () => {
        mockDb.select = vi
            .fn()
            .mockImplementationOnce(() => makeQuery({
                getResult: {
        ...
        expect(res.status).toBe(200);
        const text = await res.text();
        expect(text).toContain("<title>BobLogin - OpenShelf</title>");
    });

    it("GET /feed/orgs/:slug/collections/:cSlug/atom.xml returns 404 when org is not found", async () => {
        mockDb.select = vi.fn(() => makeQuery({ getResult: null }));
        ...
        const res = await app.request(
            "http://localhost/feed/orgs/missing-org/collections/any-col/atom.xml",
            {},
            env as any,
        );

        expect(res.status).toBe(404);
        await expect(res.json()).resolves.toMatchObject({ error: "Org not found" });
    });
    ...
    it("GET /feed/orgs/:slug/collections/:cSlug/atom.xml returns 404 when collection is private", async () => {
        ...
        expect(res.status).toBe(404);
        await expect(res.json()).resolves.toMatchObject({ error: "Collection not found" });
    });

    it("GET /feed/orgs/:slug/collections/:cSlug/atom.xml returns 404 when collection record is null", async () => {
        mockDb.select = vi
            .fn()
            .mockImplementationOnce(() => makeQuery({
                getResult: {
        ...
        const res = await app.request(
            "http://localhost/feed/orgs/lab/collections/no-such-col/atom.xml",
            {},
            env as any,
        );
    ...
    it("GET /feed/users/:id/atom.xml sets content-type to application/atom+xml", async () => {
        mockDb.select = vi
            .fn()
            .mockImplementationOnce(() => makeQuery({
                getResult: {
    ...
    it("GET /feed/orgs/:slug/atom.xml accepts single-character slug", async () => {
        mockDb.select = vi
            .fn()
            .mockImplementationOnce(() => makeQuery({
                getResult: {
        ...
        const res = await app.request("http://localhost/feed/orgs/x/atom.xml", {}, env as any);
