...
describe("papers routes", () => {
    ...
    it("PATCH /api/papers/:id validates tags property types correctly", async () => {
        const token = await createTestJWT({ sub: "user-1", githubId: "123", name: "Uploader" });
        const app = await createTestApp();
        const env = createTestEnv();
        ...
        expect(body.error).toContain("tags must be an array or null");
        ...
        expect(body.error).toContain("each tag must be a string");
        ...
        res = await app.request(
            "/api/papers/paper-1",
            {
                method: "PATCH",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                body: JSON.stringify({ tags: ["a-very-long-tag-that-exceeds-the-maximum-tag-length-of-64-characters-by-quite-a-bit-this-is-too-long"] }),
            },
            env,
        );
        expect(res.status).toBe(400);
        body = (await res.json()) as any;
        expect(body.error).toContain("each tag must be 64 chars or less");
    });

    it("PATCH /api/papers/:id updates tags ignoring empty ones and normalizing", async () => {
        const token = await createTestJWT({ sub: "user-1", githubId: "123", name: "Uploader" });
        const where = vi.fn(async () => undefined);
        const set = vi.fn(() => ({ where }));
        mockDb.select = vi
        ...
        const res = await app.request(
            "/api/papers/paper-1",
            {
                method: "PATCH",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                body: JSON.stringify({ tags: ["Valid-Tag", "", "   ", "another-tag"] }),
            },
            env,
        );
        expect(res.status).toBe(200);
        ...
        expect(set).toHaveBeenCalledWith(
            expect.objectContaining({
                tags: JSON.stringify(["Valid-Tag", "another-tag"]),
            }),
        );
    });

    it("PATCH /api/papers/:id handles clearing tags", async () => {
        const token = await createTestJWT({ sub: "user-1", githubId: "123", name: "Uploader" });
        const where = vi.fn(async () => undefined);
        const set = vi.fn(() => ({ where }));
        mockDb.select = vi
