...
describe("papers routes", () => {
    ...
    it("POST /api/papers creates paper with org_only visibility", async () => {
        ...
        const orgInsert = batchArgs.find((q: any) => q && q.type === 'insert' && q.data && q.data.orgId === "org-1");
        expect(orgInsert).toBeDefined();
    });

    it("POST /api/papers logs cleanup failures when R2 and D1 cleanup both reject on upload error", async () => {
        const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
        const token = await createTestJWT({ sub: "user-1", githubId: "123", name: "Uploader" });
        const app = await createTestApp();
        const env = createTestEnv();

        // Make BUCKET.put succeed so uploadedKeys gets populated, then fail on delete
        const r2Store = new Map<string, unknown>();
        const mockBucket = {
            put: async (key: string, value: unknown) => { r2Store.set(key, value); },
            delete: vi.fn(async (_key: string) => { throw new Error("R2 delete failed"); }),
            get: async (key: string) => r2Store.has(key) ? { key, value: r2Store.get(key) } : null,
        };

        // Make db.batch throw so we enter the catch block
        mockDb.batch = vi.fn(async () => { throw new Error("DB batch failed"); });
        // Make db.delete also reject for D1 cleanup failures
        mockDb.delete = vi.fn(() => ({ where: vi.fn(async () => { throw new Error("D1 delete failed"); }) }));
        ...
        try {
            await app.request(
                ...
                { ...env, BUCKET: mockBucket } as any,
            );
        } catch {
            thrown = true;
        }

        // The error should propagate (either thrown or 500 response) and cleanup failures logged
        const cleanupErrorCalls = consoleErrorSpy.mock.calls.filter(
            (args) => typeof args[0] === "string" && args[0].includes("Cleanup failures"),
        );
        expect(cleanupErrorCalls.length).toBeGreaterThan(0);

        consoleErrorSpy.mockRestore();
    });

    it("POST /api/papers rethrows original error even when R2 and D1 cleanup succeed", async () => {
        const token = await createTestJWT({ sub: "user-1", githubId: "123", name: "Uploader" });
        const app = await createTestApp();
        const env = createTestEnv();

        // Make db.batch throw the original error
        mockDb.batch = vi.fn(async () => { throw new Error("original upload error"); });
        ...
        // The original error causes a 500 response
