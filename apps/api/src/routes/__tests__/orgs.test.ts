...
describe("orgs routes", () => {
    ...
    describe("Additional edge cases", () => {
        ...
        it("POST /api/orgs handles race condition on insert (unique constraint)", async () => {
            const token = await createTestJWT({ sub: "user-1" });
            queueSelectResponses([
                { getResult: null }, // slug uniqueness check passes
            ]);
            mockDb.insert = vi.fn(() => ({
                values: vi.fn(async () => {
                    throw new Error("D1_ERROR: UNIQUE constraint failed: orgs.slug");
                }),
            }));

            const app = await createTestApp();
            const res = await app.request("http://localhost/api/orgs", {
                method: "POST",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                body: JSON.stringify({ name: "Race", slug: "race" }),
            }, createTestEnv() as any);
        ...
        it("PATCH /api/orgs/:slug/members/:userId allows demoting an admin if they are not the last one", async () => {
            const token = await createTestJWT({ sub: "user-1" });
            queueSelectResponses([
                { getResult: { id: "org-1", slug: "my-lab" } },
                { getResult: { role: "admin" } }, // actor
                { getResult: { role: "admin" } }, // target
            ]);
            ...
            const app = await createTestApp();
            const res = await app.request("http://localhost/api/orgs/my-lab/members/user-2", {
                method: "PATCH",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                body: JSON.stringify({ role: "member" }),
            }, createTestEnv() as any);
        ...
        it("DELETE /api/orgs/:slug/members/:userId allows removing an admin if they are not the last one", async () => {
            const token = await createTestJWT({ sub: "user-1" });
            queueSelectResponses([
                { getResult: { id: "org-1", slug: "my-lab" } },
                { getResult: { role: "admin" } }, // actor
        ...
        it("GET /api/orgs/:slug/papers handles invalid JWT gracefully (shows public papers)", async () => {
            queueSelectResponses([
                { getResult: { id: "org-1", slug: "my-lab" } },
                { allResult: [{ paperId: "p-pub" }] },
                { allResult: [{ id: "p-pub", title: "Public", visibility: "public" }] },
            ]);

            const app = await createTestApp();
            const res = await app.request("http://localhost/api/orgs/my-lab/papers", {
            ...
            expect(body.papers[0].id).toBe("p-pub");
        });

        it("POST /api/orgs returns 400 for invalid description", async () => {
            const token = await createTestJWT({ sub: "user-1" });
            const app = await createTestApp();
            const res = await app.request("http://localhost/api/orgs", {
                method: "POST",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                body: JSON.stringify({ name: "Lab", slug: "lab", description: "a".repeat(1001) }),
            }, createTestEnv() as any);
            expect(res.status).toBe(400);
        });

        it("PATCH /api/orgs/:slug returns 409 if new slug is already taken", async () => {
            const token = await createTestJWT({ sub: "user-1" });
            queueSelectResponses([
                { getResult: { id: "org-1", slug: "old-slug" } },
                { getResult: { role: "admin" } },
                { getResult: { id: "org-2", slug: "new-slug" } }, // already taken
            ]);

            const app = await createTestApp();
            const res = await app.request("http://localhost/api/orgs/old-slug", {
                method: "PATCH",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        ...
        it("POST /api/orgs/:slug/members returns 400 for invalid role", async () => {
            const token = await createTestJWT({ sub: "user-1" });
            queueSelectResponses([
            ...
            const app = await createTestApp();
            const res = await app.request("http://localhost/api/orgs/lab/members", {
                method: "POST",
                headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                body: JSON.stringify({ userId: "u2", role: "owner" }), // owner is not allowed in POST
            }, createTestEnv() as any);

            expect(res.status).toBe(400);
        });

        it("POST /api/orgs/:slug/papers returns 403 if user is neither admin nor author", async () => {
        ...
        it("POST /api/orgs/:slug/members handles race condition on insert (unique constraint)", async () => {
            ...
            queueSelectResponses([
                { getResult: { id: "org-1", slug: "lab" } },
                { getResult: { role: "admin" } },
                { getResult: { id: "user-2" } }, // user exists
                { getResult: null }, // member check passes
            ]);
            mockDb.insert = vi.fn(() => ({
                values: vi.fn(async () => {
                    throw new Error("UNIQUE constraint failed: org_members.org_id, org_members.user_id");
                }),
            }));
