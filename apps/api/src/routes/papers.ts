...
async function prepareUploadEntries(
  ...
  const validationResults = await Promise.all(
    uploads.map((u) =>
      validateMagicNumbers(u.file, u.file.type).then((isValid) => ({
        isValid,
        file: u.file,
      })),
    ),
  ...
  for (const result of validationResults) {
    if (!result.isValid) {
      console.error(
        `Magic number validation failed for file ${result.file.name} (declared: ${result.file.type})`,
      );
      return {
        errorResponse: c.json(
          {
            error: `File ${result.file.name} does not match expected format for ${result.file.type}`,
          },
          400,
        ),
      };
...
papersRoute.delete("/:id", authMiddleware, async (c) => {
  ...
  await deleteKeysInBatches(c.env.BUCKET, keys, (info) => {
    console.error(
      "R2 deletion failed (non-fatal, DB deletion continues):",
      info.error,
      info.chunkSample,
    );
  });
