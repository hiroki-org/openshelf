...
function purgeExpiredTokenCache(now: number): void {
    ...
    for (const [cachedToken, entry] of tokenCache.entries()) {
        if (entry.expiresAt <= now) {
            ...
            // Bound cleanup work by stopping after 20 consecutive valid entries.
            // Expired entries found before that boundary reset the scan, so mildly
            // out-of-order JWT expirations are still removed. Expired entries behind
            // a longer run of valid tokens may remain until a later cache purge.
            if (++checked >= 20) {
                break;
            }
        }
