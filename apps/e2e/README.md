...
CIではGitHub-hosted runner上でPlaywrightのブラウザバイナリをキャッシュしつつ、`npx playwright install chromium`で必要分だけを復元します。Ubuntu runner側に入っているシステム依存をそのまま使うことで、`--with-deps` によるapt解決の長い揺れを避けています。
CIでは2 shardに分割してPlaywrightを実行し、各 shard ごとに以下をArtifactsとして保存します。

- `playwright-report-*`: HTMLレポート
- `playwright-test-results-*`: JUnit結果、失敗時のスクリーンショット、trace、video

これにより、CI上でのみ再試行を増やしつつ、失敗時の調査材料を取りこぼさず回収できます。
