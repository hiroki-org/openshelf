...
describe("UploadPage", () => {
  ...
  it("blocks submit while organizations are still loading", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/タイトル/i), {
      target: { value: "Loading org paper" },
    });
    fireEvent.change(screen.getByLabelText("アップロードファイル"), {
      target: {
        files: [new File(["F"], "f.pdf", { type: "application/pdf" })],
    ...
    expect(
      await screen.findByText(/組織を読込中です。しばらくお待ちください/i),
    ).toBeInTheDocument();

    resolveOrgs?.(
      new Response(
        JSON.stringify({
          organizations: [{ id: "org-1", name: "Org 1", slug: "org-1", role: "member" }],
        }),
        { status: 200 },
      ),
    );
  ...
  it("blocks org_only submit when no organizations are available", async () => {
    ...
    await screen.findByDisplayValue("組織がありません");
    ...
    expect(
      await screen.findByText(/組織がありません。別の公開範囲を選択してください/i),
    ).toBeInTheDocument();
  });
