...
const originalMock = vi.mocked(apiFetch).getMockImplementation();
  ...
  it("switches to the general tab", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "メンバー" }));
    await screen.findByText("メンバーを追加");

    // Click General tab
    fireEvent.click(screen.getByRole("button", { name: "一般" }));
    await screen.findByLabelText("組織名");
  });

  it("clears paper search results when query is too short", async () => {
    setupOrgApiMock({
      org: { id: "o1", slug: "demo-org", name: "Org", description: null },
      members: [{ userId: "owner-1", role: "owner", name: "o", displayName: "O", avatarUrl: null, githubId: "o" }],
      papers: [],
    ...
    // First, enter a valid query to populate results
    const originalMock = vi.mocked(apiFetch).getMockImplementation();
    vi.mocked(apiFetch).mockImplementation(async (url, init) => {
      if (url.includes("/api/papers") && !url.includes("orgs")) {
         return jsonResponse({ papers: [{ id: "p1", title: "Paper 1" }] });
      }
    ...
    // Then, clear it to trigger q.length < 2
  ...
  it("shows error when member fetch fails", async () => {
    ...
    vi.mocked(apiFetch).mockImplementation(async (url) => {
      ...
      if (urlStr.endsWith("/members")) return jsonResponse({ error: "Fail" }, 400);
      if (urlStr.endsWith("/papers")) return jsonResponse({ papers: [] });
      if (urlStr.includes("/api/orgs/")) return jsonResponse({ org: { id: "o1", slug: "demo-org", name: "Demo" } });
      return jsonResponse({});
    });
