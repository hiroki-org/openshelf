...
3. **Environment Variables の設定**
   ...
   - `FRONTEND_URL`: Vercel 上の自身の URL (例: `https://openshelf.vercel.app`)
...
2. **環境変数の設定**:
   Cloudflare Workers では、機密性の高い値は **Secrets** として、それ以外は **Vars** として設定します。
   ...
   - **Vars (wrangler.toml の [vars] またはデプロイ時に設定)**:
     - `GITHUB_CLIENT_ID`
     - `FRONTEND_URL` (Vercel の Web URL)
   ...
   > [!NOTE]
   > `process.env` は Cloudflare Workers 環境では `c.env` に自動マッピングされないため、Wrangler を通じて明示的に注入する必要があります。
