...
## 2025-06-12 - Badge Snippet Copy Button Accessible Name
**Learning:** リスト内の複数の要素に対して同一の視覚的テキスト（例：「Copy」）を持つアクションボタンを配置する場合、スクリーンリーダーユーザーがどの項目のアクションかを判別できるように、`aria-label`にコンテキスト（例：`${snippet.label}`）を含める必要があります。また、WCAG 2.5.3（Label in Name）に準拠するため、`aria-label`には視覚的テキスト（この場合は「Copy」）を必ず含める必要があります。
**Action:** 今後、リスト内で同じテキストのアクションボタンを複数配置する場合は、必ずコンテキストと視覚的テキストの両方を含んだ `aria-label` を設定します。
