...
## 2025-05-25 - Copy Button Accessible Names Improvement
**Learning:** 複数の「Copy」ボタンが同じ画面上に存在する場合（例えば異なる形式のスニペットをコピーするボタン群など）、スクリーンリーダーユーザーが各ボタンを区別できるように、文脈に応じた `aria-label` を設定することが重要です。また、WCAG 2.5.3 (Label in Name) に準拠するため、`aria-label` には画面上で視覚的に表示されているテキスト（この場合は "Copy"）を必ず含める必要があります。
**Action:** 今後、リスト内などで同一テキストを持つ複数のアクションボタンを実装・改善する際は、必ずアイテムの固有情報を視覚的なテキストと組み合わせた `aria-label` を設定します。
