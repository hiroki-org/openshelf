...
## 2025-05-23 - Character Counter Visual Feedback
**Learning:** 文字数カウンターにおいて、単に現在文字数と最大文字数を表示するだけでなく、上限（例: maxLengthの90%）に達した際に動的に色を警告色（赤など）に変更することで、ユーザーが予期せずハードリミットに達してエラーになるのを防ぐ、よりプロアクティブで親切なマイクロUXが実現できる。
**Action:** 今後、長文入力を伴うフォーム要素（textareaなど）に対して文字数カウンターを実装する際は、しきい値を設けて視覚的フィードバック（警告色への変化）を組み込むことを標準パターンとする。
