...
## 2025-06-04 - Character Counter Accessibility Over-announcement
**Learning:** 文字数カウンター（例: 組織作成やコレクション作成フォーム）において、カウンター要素に `aria-live="polite"` と `aria-atomic="true"` を付与すると、ユーザーのキーストロークごとにスクリーンリーダーが毎回「10 / 500」のように読み上げてしまい、非常にノイジーで煩わしい体験になります。
**Action:** 今後、入力のたびに更新される文字数カウンターには `aria-live` を使用せず、入力フィールド側に `aria-describedby` を付与してカウンター要素と紐付けるアプローチ（一度だけ読み上げられる、あるいは入力前にコンテキストとして伝わる）を標準とします。
