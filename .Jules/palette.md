...
## 2026-06-25 - Form Validation and Error Message Accessibility
**Learning:** 動的に変化するフォームの検証ステータス（例：slugの重複チェック）や、非同期アクションの送信後に表示されるエラーメッセージにおいては、スクリーンリーダーユーザーへの適切なフィードバックが不可欠です。検証ステータスは `aria-describedby` で入力フィールドと紐付け、ステータスのコンテナには `aria-live="polite"` を設定することで、入力中や結果取得時の変更をスムーズに伝達できます。また、送信エラーのコンテナには `role="alert"` を付与することで、エラー発生時に即座に読み上げられるようになります。
**Action:** 今後、動的なバリデーションステータスやフォームの送信エラーを表示するUIを構築・改善する際は、必ず `aria-describedby` と `aria-live` の組み合わせ、および `role="alert"` を用いてアクセシビリティを確保します。
