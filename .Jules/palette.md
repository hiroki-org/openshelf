...
## 2026-06-15 - Dynamic Validation Status Accessibility
**Learning:** slug などのリアルタイムなバリデーション結果（使用可能、既に使用されています等）を表示する際、スクリーンリーダーユーザーが入力中に状態変化を把握できるように、入力要素（input）に `aria-describedby` を付与してステータス要素を紐付け、ステータス要素自身に `aria-live="polite"` を設定することが重要です。これにより、目視だけでなく音声でも即座にフィードバックが得られ、より良い体験になります。
**Action:** 今後、動的なバリデーションを持つフォーム入力を実装・改善する際は、入力フィールドの `aria-describedby` と、ステータス表示領域の `aria-live="polite"` を組み合わせるアプローチを徹底します。
