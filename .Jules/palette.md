...
## 2024-06-09 - [動的エラーメッセージのアクセシビリティ改善]
**Learning:** Reactの条件付きレンダリング（例：`{error && <p>{error}</p>}`）で動的にDOMに追加されるエラーメッセージは、視覚的には明らかでも、スクリーンリーダーには自動的に通知されないことが多い。そのため、エラーが発生したことに気づかないユーザーが出てしまう可能性がある。
**Action:** 動的に表示されるエラーメッセージ（フォームのバリデーションエラーやAPIのレスポンスエラーなど）を内包する要素には、必ず `role="alert"` （または状況に応じて `aria-live="assertive"` や `aria-live="polite"`）を付与する。これにより、エラーメッセージがDOMに挿入された瞬間に、スクリーンリーダーがそれをユーザーに読み上げるようになる。
