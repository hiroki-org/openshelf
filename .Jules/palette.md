...
## 2026-06-03 - Form Input Character Counter Accessibility Improvement
**Learning:** 入力フィールド（今回は`description`など）に対する文字数カウンター実装において、該当のカウンター要素に `aria-live="polite"` と `aria-atomic="true"` を設定すると、ユーザーが文字を入力するたびにスクリーンリーダーが文字数を読み上げてしまい、非常に煩わしい体験を引き起こします。入力項目には `aria-describedby` 属性のみで紐付けを行い、動的な読み上げを抑制することがベストプラクティスです。
**Action:** 今後、動的な文字数制限カウンターなどのUI要素を実装する際は、読み上げ頻度を考慮し、原則として `aria-live` の使用を避け、`aria-describedby` による静的な紐付けのみでアクセシビリティを確保します。
