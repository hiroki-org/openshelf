...
## 2026-05-19 - Accessible Error States
**Learning:** 動的なエラーメッセージやステータスメッセージを表示する際、単なる `<p>` や `<div>` 要素ではなく、`role="alert"` (エラーの場合) または `role="status"` (成功等の場合) を指定することで、スクリーンリーダーユーザーに対して状態変化を即座に伝えることができます。また、インラインでローディングスピナーを実装する際は、`aria-hidden="true"` を付与した共通コンポーネント (例: `<Spinner />`) に置き換えることで、コードの再利用性を高めるだけでなく、スクリーンリーダーによる不要なDOMの読み上げを防ぐことができます。
**Action:** 今後、非同期処理の結果をユーザーに表示するコンポーネント (エラー表示やステータス表示) を実装・改善する際は、必ず適切な `role` 属性を付与し、ローディング表示にはアクセシブルな共通スピナーコンポーネントを使用します。
