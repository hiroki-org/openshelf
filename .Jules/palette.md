...
**Learning:** 招待一覧のような動的なリストにおいて、「承認」や「拒否」といった複数の同一アクションボタンが存在する場合、それぞれのボタンに `aria-label="\${title}への招待を承認"` のような具体的なコンテキストを持たせたアクセシブルな名前を設定することが重要です。これによりスクリーンリーダーユーザーがどの項目のアクションか明確に判別できるようになります。また、ボタンの `aria-label` を変更した際は、テストコード内で使用されている `getByRole('button', { name: '承認' })` などのクエリも新しいアクセシブルな名前に追従して修正する必要があります。
**Action:** 今後、動的リスト内のアクションボタンを実装または改善する際は、必ずアイテム固有のコンテキストを含めた `aria-label` を設定し、関連する単体テストも合わせて修正します。
