...
**Learning:** アイコンやコピーなどの汎用的なアクションを実行するボタンが複数リストされている場合、スクリーンリーダーユーザーがどの項目のアクションかを識別できるように、`aria-label` に具体的なコンテ キスト（例: `Copy ${snippet.label}`）を含めることが重要です。同時 に、表示されているテキスト（この場合は 'Copy'）を `aria-label` に 含めることで WCAG 2.5.3（Label in Name）に準拠します。
**Action:** 今後、リスト内やコンテキストが不明瞭なボタン（特に同じテキストラベルを持つ複数のボタン）を実装・改善する際は、必ず `aria-label` に詳細な説明を含めるようにします。また、以前試みた「文字数カウンターの aria-live 属性の削除」は、スクリーンリーダーユーザー へのフィードバックを完全に奪うことになりアクセシビリティを低下させる（リグレッション）ため、行わないよう注意します。

## 2026-06-09 - 動的なローディング状態のアクセシビリティ改善
**Learning:** 動的なローディング状態（例：非同期での候補取得中）をスクリーンリーダーに確実に読み上げさせるためには、`aria-live="polite"` 属性を持つ要素を条件付きでレンダリング（例：`{loading && <div aria-live="polite">...</div>}`）してはいけません。コンテナ要素はDOMに永続的にマウントしておき、テキスト内容のみを条件付きで切り替える必要があります。
**Action:** 今後、動的なステータスメッセージやローディング表示を実装する際は、永続的な `aria-live` コンテナ（`sr-only` クラスなどで 視覚的に隠す）を使用し、視覚的な表示用要素には `aria-hidden="true"` を指定して二重読み上げを防止するパターンを使用します。
