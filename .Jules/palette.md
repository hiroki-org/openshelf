...
## 2025-06-16 - Accessible Names for Dynamic List Actions
**Learning:** 招待一覧のような動的なリストにおいて、「承認」や「拒否」といった複数の同一アクションボタンが存在する場合、それぞれのボタンに `aria-label="承認: \${title}への招待"` のような具体的なコンテキスト を持たせたアクセシブルな名前を設定することが重要です。この際、WCAG 2.5.3 (Label in Name) に準拠するため、視覚的なテキスト（ 「承認」や「拒否」）をアクセシブルな名前の先頭に配置することで、音声入力ユーザーがボタンを容易にターゲットできるようになります。また、ボタンの `aria-label` を変更した際は、テストコード内で使用されている `getByRole('button', { name: '承認' })` など のクエリも新しいアクセシブルな名前に追従して修正する必要があります。
**Action:** 今後、動的リスト内のアクションボタンを実装または改善する際は、必ず視覚的テキストを先頭に含めたアイテム固有のコンテキストを持つ `aria-label` を設定し、関連する単体テストも合わせて修正します。
