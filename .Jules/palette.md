...
## 2026-06-30 - Markdown Editor Tab Accessibility
**Learning:** `Write` や `Preview` といったシンプルなテキストラベルを持つタブ（`role="tab"`）であっても、コンテキストを含めた詳細な `aria-label`（例: `Write: Edit Markdown`）を付与することで、スクリーンリーダーユーザーにとって現在どの領域を操作・表示しようとしているかが明確になります。また、表示テキストと同じテキストを `aria-label` の先頭に含めることで WCAG 2.5.3 (Label in Name) の原則を満たし、アクセシビリティを保つことができます。
**Action:** 今後、タブやトグルボタンなどの切り替えインタラクションを実装する際は、単語だけでなく具体的なコンテキストを示す `aria-label` を追加し、テスト側の `getByRole` 等のマッチャーは完全一致の文字列ではなく、正規表現（例: `/Write/`）を使用して変更に強い構造にします。
