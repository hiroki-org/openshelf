...
## 2026-06-10 - Form Error Rendering Accessibility
**Learning:** フォーム送信時などに動的に表示されるエラーメッセージ（例：`{error && <p>{error}</p>}`）において、コンテナ要素に `role="alert"` が設定されていないと、スクリーンリーダーが新しいエラーの出現を自動的に検知して読み上げることができません。
**Action:** 今後、検証エラーや送信エラーなど、ユーザーの対応を促す動的なメッセージを表示する際は、表示要素に必ず `role="alert"` を設定し、アクセシビリティを確保します。
