...
## 2026-06-11 - Dynamic Error Message Accessibility Improvement
**Learning:** フォーム送信時やバリデーション時に条件付きで表示される動的なエラーメッセージコンテナには、`role="alert"` を設定することで、エラーがDOMに現れた瞬間にスクリーンリーダーが自動的かつ即座にそれを読み上げ、視覚障害を持つユーザーに重要な状態の変化を確実に伝えることができます。
**Action:** 今後、動的に表示・非表示が切り替わるエラーメッセージや警告などの重要な状態変更要素を実装する際は、常に `role="alert"` を付与してアクセシビリティを確保します。
