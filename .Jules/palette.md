...
## 2026-06-27 - Dynamic Form Validation & Error Accessibility
**Learning:** 入力フィールドに関連する動的なバリデーションステータス（例：スラグの「確認中」「使用可能」など）やフォームの送信エラーを表示する際、スクリーンリーダーユーザーに適切に状態を伝えるために、入力フィールドに `aria-describedby` を使用してステータス要素を紐付け、ステータス要素自身には `aria-live="polite"` を設定することが重要です。また、フォーム送信エラーを表示する要素には `role="alert"` を付与することで、エラー発生時に自動的かつ即座に読み上げられるようにする必要があります。
**Action:** 今後、動的な入力バリデーションやフォームのエラーメッセージを実装・改善する際は、必ず `aria-describedby`、`aria-live="polite"`、および `role="alert"` を組み合わせて、リアルタイムなフィードバックがスクリーンリーダーで適切に読み上げられるようにします。
