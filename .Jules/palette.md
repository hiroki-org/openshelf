...
## 2026-05-27 - Character Counter aria-live Removal
**Learning:** 文字数カウンターにおいて、タイピングのたびにカウントが更新される要素に `aria-live="polite"` を設定すると、スクリーンリーダーがタイピングのたびに頻繁に読み上げてしまい、深刻なアクセシビリティ（UX）の問題を引き起こします。入力フィールドに対する文字数カウンターなど、付加的な動的情報を提供する際は `aria-live` は使用せず、入力フィールドに `aria-describedby` 属性を付与してカウンター要素を紐付けるアプローチを採用すべきです。
**Action:** 今後、動的な文字数カウンターなどのUIを実装・修正する際は、`aria-live` は使用しないことを徹底します。

## 2026-05-27 - [Invites Page] Empty State Navigation
...
## 2026-05-31 - Empty State Call to Actions Improvement
**Learning:** 空状態（Empty State）において「まだありません」とだけ表示するのではなく、次のアクション（コレクションを作成する、組織に成果物を追加するなど）への導線となるCTA（Call to Action）リンクを提供することで、ユーザーが「行き止まり」を感じることなく回遊性を維持できることが確認できました。特に、ユーザー権限（例: `isSelf`）に応じた条件付きCTAを配置することが有用です。
**Action:** 今後、リストやコレクションが空である状態のUIを設計・修正する際は、単に「何もない」ことを伝えるだけでなく、必ずユーザーを次の主要な動線に導くCall To Action (CTA) リンクやボタンを配置します。
