...
## 2026-06-21 - Form Error Message Accessibility Improvement
**Learning:** 動的にレンダリングされるフォームのエラーメッセージ（例：`{error && <div>{error}</div>}`）において、コンテナ要素に `role="alert"` 属性を付与することで、スクリーンリーダーを利用しているユーザーに対してエラーの発生とその内容を即座にかつ自動的に読み上げさせることができ、アクセシビリティが劇的に向上します。
**Action:** 今後、ユーザー入力のバリデーションエラーや非同期処理の失敗など、動的にDOMに追加される警告メッセージ・エラー表示UIを実装する際は、常に `role="alert"` 属性を含めるようにします。
