...
## 2026-06-23 - Dynamic Error Messages Accessibility
**Learning:** 動的にレンダリングされるエラーメッセージ（例: フォームのバリデーションエラーや送信失敗メッセージ）は、DOM に挿入された際にスクリーンリーダーが自動的に読み上げないことがあります。
**Action:** 今後、条件付きでレンダリングされるステータスメッセージやエラーメッセージを実装・改善する際は、メッセージを囲むコンテナ要素に `role="alert"` を追加し、出現時に即座に読み上げられるようにします。
