...
## 2026-06-05 - BadgeSnippet Accessible Name
**Learning:** バッジのコードなどをクリップボードにコピーするようなアクションボタン（例: "Copy" というテキストだけのボタン）において、対象のコードの種類（Markdown, HTML等）が明確でない場合、スクリーンリーダーユーザーにとって操作の結果が予測しづらくなります。文脈を補うために、動的な `aria-label`（例: "[フォーマット]のバッジコードをコピー"）を付与することでアクセシビリティが向上します。
**Action:** 今後、リスト内や複数存在する同じテキストのアクションボタン（"Copy", "Delete" など）を実装・改善する際は、必ず対象のアイテムやフォーマットを示す詳細な `aria-label` を設定します。
