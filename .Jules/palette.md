...
**Learning:** アイコンやコピーなどの汎用的なアクションを実行するボタンが複数リストされている場合、スクリーンリーダーユーザーがどの項目のアクションかを識別できるように、`aria-label` に具 体的なコンテキスト（例: `Copy ${snippet.label}`）を含めること が重要です。同時に、表示されているテキスト（この場合は 'Copy' ）を `aria-label` に含めることで WCAG 2.5.3（Label in Name）に準拠します。
**Action:** 今後、リスト内やコンテキストが不明瞭なボタン（特に同じテキストラベルを持つ複数のボタン）を実装・改善する際は、必ず `aria-label` に詳細な説明を含めるようにします。また、以前試みた「文字数カウンターの aria-live 属性の削除」は、スクリーン リーダーユーザーへのフィードバックを完全に奪うことになりアクセシビリティを低下させる（リグレッション）ため、行わないよう注意します。

## 2025-05-29 - Dynamic Error Message Accessibility Improvement
**Learning:** フォーム送信やバリデーションエラーなど、動的に出現するエラーメッセージに対しては、そのコンテナに `role="alert"` 属性を追加することが重要です。これにより、DOM にエラーメッセージが表示された瞬間、スクリーンリーダーが自動的かつ即座にその内容を読み上げるようになり、視覚に障害のあるユーザーにもエラー状態が正確に伝わります。
**Action:** 今後、動的に表示・非表示が切り替わるエラーメッセージ（例：`{error && <div>{error}</div>}`）を実装する際は、必ず 対象要素に `role="alert"` を設定し、アクセシビリティを確保します。
