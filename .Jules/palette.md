...
## 2026-05-26 - Accessible Icon Buttons and Tooltips Enhancement
**Learning:** アイコンや記号のみのボタン（「↑」「↓」「✕」など）に対して `aria-label` を設定してスクリーンリーダー向けのアクセシビリティを確保するだけでなく、非活性状態でない通常のボタンに対しても `title` 属性を併用することで、マウスユーザーがホバー時にアクションの目的（「上に移動」「削除」など）を視覚的に把握できるようになり、UXがさらに向上します。
**Action:** 今後、アイコンのみのインタラクティブ要素を実装・改善する際は、必ず `aria-label` と同等の分かりやすい説明を `title` 属性としても追加するようにします。
