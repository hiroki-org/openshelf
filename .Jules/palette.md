...
## 2026-06-22 - Slug Validation Accessibility
**Learning:** 動的に検証結果が変わるフォーム入力（スラッグの可用性など）では、スクリーンリーダーに変更を伝えるためにフィードバック要素に `aria-live="polite"` を設定し、それを入力フィールドに `aria-describedby` で紐付ける必要があります。また、`aria-live` を設定するラッパー要素は条件付きレンダリングでDOMから出し入れするのではなく、常にマウントしておきテキストだけを更新することが重要です。
**Action:** 今後、非同期で状態やメッセージが動的に変わるフォーム入力（スラッグチェックやパスワード強度など）を実装する際は、入力フィールドに `aria-describedby` を指定し、常駐するメッセージコンテナに `aria-live="polite"` を設定して、アクセシブルなフィードバックを提供するようにします。
