...
## 2026-06-23 - MarkdownEditor Tabs Accessibility
**Learning:** `role="tablist"` 内の個々のタブに対して、画面上のテキストと重複する `aria-label` (例えば "Write: Edit Markdown") を追加すると、スクリーンリーダーの出力が冗長になり、かえってアクセシビリティを損なう可能性があります。タブリスト自体のコンテキスト (`aria-label="Markdown editor mode"`) が十分に設定されていれば、個別のタブには表示テキスト ("Write" / "Preview") のみを持たせるべきです。
**Action:** 今後、`role="tab"` を持つ要素のアクセシビリティを改善する際は、すでに親要素（`tablist`）で文脈が提供されているかを確認し、無意味に冗長な `aria-label` を追加しないように注意します。
