...
## 2026-06-25 - Dynamic Form Validation Accessibility
**Learning:** 動的に入力値を検証し、結果のステータス（例: "確認中...", "使用可能", "使用済み"）を非同期に表示するフォーム要素において、スクリーンリーダーユーザーがフォーカスを外すことなくステータスの変化を把握できるようにするためには、対象の入力要素（`<input>`など）に `aria-describedby="status-element-id"` を付与し、ステータスを表示する要素に `id="status-element-id"` と `aria-live="polite"` を設定することが非常に効果的です。これにより、ユーザーはタイピング中やフォーカス中にステータスの変化を安全に知ることができます。
**Action:** 今後、非同期または動的にステータスが更新されるフォーム入力を実装・改善する際は、必ず入力要素に `aria-describedby` を付与し、ステータスメッセージ要素を `aria-live="polite"` でマークアップして、視覚情報だけでなくスクリーンリーダーでも自然に情報が伝わるようにします。
