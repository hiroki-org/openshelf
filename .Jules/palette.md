...
## 2024-06-09 - 動的なローディング状態のアクセシビリティ改善
...
**Action:** 今後、動的なステータスメッセージやローディング表示を実装する際は、永続的な `aria-live` コンテナ（`sr-only` クラスなどで視覚的に隠す）を使用し、視覚的な表示用要素には `aria-hidden="true"` を指定して二重読み上げを防止するパターンを使用します。
