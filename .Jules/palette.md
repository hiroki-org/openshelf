...
## 2026-06-14 - Dynamic Error Message Accessibility (role="alert")
**Learning:** フォームや入力要素において、バリデーションエラーや送信エラーのメッセージが動的にDOMに追加される際（例: `{error && <div>{error}</div>}`）、その要素に `role="alert"` 属性を付与することで、スクリーンリーダーがエラー発生時にその内容を即座に自動的に読み上げるようになり、視覚障害のあるユーザーにとってのアクセシビリティが大幅に向上します。
**Action:** 今後、ユーザーの操作に応じて動的に表示されるエラーメッセージ（フォームのバリデーションエラーやAPIの送信エラーなど）を実装・改善する際は、必ず `role="alert"` 属性を追加するようにします。
