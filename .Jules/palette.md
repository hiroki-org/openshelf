...
**Learning:** 動的なフォーム検証（例: slugの重複チェック）の結果を視覚的に表示するだけでは、スクリーンリーダーユーザーに伝わらない。`aria-live="polite"`と`aria-atomic="true"`を持つ要素内にステータスメッセージを配置し、入力欄に`aria-describedby`でその要素のIDを指定することで、入力直後にステータスが適切に読み上げられ、アクセシビリティが向上する。
**Action:** 次回以降、非同期または動的に更新されるバリデーションメッセージを追加する際は、必ず`aria-describedby`と`aria-live`を組み合わせて実装する。
