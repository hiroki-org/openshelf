...
## 2026-06-27 - Form Message Accessibility
**Learning:** フォーム送信時や設定保存時に動的に表示されるエラーメッセージや成功メッセージにおいて、これらを囲む要素が単なる `<div>` や `<p>` だと、スクリーンリーダーユーザーはDOMの更新を自動的に検知・読み上げることができません。
**Action:** 今後、動的に表示されるフォームのバリデーションエラーや送信結果メッセージを追加・更新する際は、要素に必ず `role="alert"`（エラーなど重要なもの）や `role="status"`（成功など軽微なもの）を付与し、出現時に自動的にスクリーンリーダーにアナウンスされるようにします。
