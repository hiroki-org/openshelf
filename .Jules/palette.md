...
**Action:** 今後、リスト内やコンテキストが不明瞭なボタン（特に同じテキストラベルを持つ複数のボタン）を実装・改善する際は、必ず `aria-label` に表示ラベルから始まる詳細な説明を含めるようにします。また、入力フィールドの文字数カウンターなど付加的な説明テキストは `aria-live` ではなく `aria-describedby` で入力要素に紐付ける方針を優先します。
