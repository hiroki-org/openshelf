...
## 2026-05-30 - Dynamic Error Message Accessibility
**Learning:** React などで状態に応じて動的に表示されるエラーメッセージ（例：フォームバリデーションエラーや送信失敗時のメッセージ）は、単純に DOM に追加されるだけではスクリーンリーダーに読み上げられません。
**Action:** 今後、動的に表示されるエラーメッセージのコンテナ要素には、常に `role="alert"` を追加することで、メッセージが表示された瞬間にスクリーンリーダーが自動的に読み上げるように設計・実装します。
