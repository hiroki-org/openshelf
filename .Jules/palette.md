...
## 2026-06-26 - Accessible Dynamic Error Messages
**Learning:** フォームのバリデーションや送信エラーなどのメッセージが動的にDOMに追加される際、単純に条件付きレンダリング（例: `{error && <div>{error}</div>}`）を行うだけでは、スクリーンリーダーユーザーにはエラーが発生したことが伝わらないことがあります。エラーメッセージのコンテナに `role="alert"` を追加することで、メッセージが表示された瞬間に自動的かつ即座にスクリーンリーダーで読み上げられ、アクセシビリティが向上します。
**Action:** 今後、動的に表示されるフォームのエラーメッセージや警告メッセージを実装・改善する際は、必ずコンテナ要素に `role="alert"` を付与して、すべてのユーザーがエラーの発生にすぐ気づけるようにします。
