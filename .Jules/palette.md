...
## 2026-06-06 - Disambiguating List Action Buttons
**Learning:** `Copy`のような複数の同じラベルを持つボタンがリストやカードの繰り返しで表示される場合、`aria-label`に文脈（例: `${snippet.label}形式のバッジコードをコピー`）と視覚的なテキスト（例: `(Copy)`）の両方を含める必要があります。これにより、スクリーンリーダーユーザーがどの項目のコピーボタンか識別でき、かつ音声認識ユーザー向けの「WCAG 2.5.3 Label in Name」の要件も満たすことができます。
**Action:** 今後、リスト内で同じテキストを持つアクションボタン（編集、削除、コピーなど）を実装・改善する際は、必ずアイテム固有のコンテキストと視覚的ラベルの両方を含めた `aria-label` を設定します。
