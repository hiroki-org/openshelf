...
## 2026-05-24 - Tag Autocomplete Input Loading State Improvement
**Learning:** ユーザー入力に対してリアルタイムで候補をフェッチするコンポーネント（オートコンプリート入力など）において、ローディング状態を単なるテキストで示すだけでは視認性が低くなります。専用の `<Spinner />` コンポーネントを配置し、レイアウトを調整（flexboxとgapによる配置）することで、UXが向上します。また、`aria-live="polite"` を用いてスクリーンリーダーにローディングの開始と終了を通知する際は、過剰な読み上げを避けるため `role="status"` や `aria-atomic` を付与しないように注意します。
**Action:** 今後、非同期でデータを取得するインラインUIを実装する際は、テキストベースではなく、視覚的フィードバック（Spinnerなど）とアクセシビリティ対応（aria-live）を併用するパターンを適用します。
