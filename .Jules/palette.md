...
## 2026-06-14 - Snippet Copy Button Accessible Names
**Learning:** バッジ等のスニペットをコピーするボタンを複数並べる場合、単に「Copy」というラベルだけでは、スクリーンリーダーユーザーにとってどのスニペットをコピーするのか区別がつきません。
**Action:** 今後、リスト内で同じ見た目のアクションボタン（「Copy」など）を配置する際は、`aria-label="Copy Markdown"` のようにコンテキストを含めた説明的な `aria-label` を追加し、アクセシビリティを確保します。
