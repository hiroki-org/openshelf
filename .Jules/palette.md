...
## 2024-06-06 - Improve form error message visibility and accessibility
**Learning:** フォーム（コレクション作成、組織作成など）でのエラーメッセージが赤文字のみ（`<p className="text-red-600">`）となっており、視認性が低く、スクリーンリーダーでもエラー状態の変化が伝わりにくいパターンがこのアプリ内に散見されました。
**Action:** エラーメッセージに明確な背景色と境界線（`bg-red-50 border-red-200` 等）を持たせ、`role="alert"` を追加することで、視覚的・構造的により分かりやすく統一しました。今後は同様のフォームでのエラー表示時にこのパターンを利用します。
