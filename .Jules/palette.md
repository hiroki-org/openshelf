...
## 2025-06-15 - Identical Text Buttons Accessibility Improvement
**Learning:** 同じテキスト（例：「Copy」「編集」「削除」など）を持つボタンがリストなどの繰り返し要素内に複数存在する場合、視覚的には文脈からどの項目に対するアクションか推測可能ですが、スクリーンリーダーユーザーにとっては文脈が欠落するため区別が困難になります。そのため、WCAG 2.5.3 (Label in Name) に準拠しつつ、対象項目の情報（例：`${snippet.label}`）を含めた具体的な `aria-label` を設定することが重要です。
**Action:** 今後、リストや繰り返し要素内で同一テキストのボタンを実装・改善する際は、必ず対象の項目名などの文脈情報を含み、かつ可視テキストも含む `aria-label` を設定します。
