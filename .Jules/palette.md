...
## 2026-06-26 - Form Validation Accessibility
**Learning:** 動的に検証されるフォーム入力（スラッグの利用可能チェックなど）や、フォーム送信時のエラーメッセージは、視覚的には変化が分かってもスクリーンリーダーには伝わりにくい場合があります。入力フィールドに `aria-describedby` を設定してステータス要素と紐付け、その要素に `aria-live="polite"` を付与することで、状態の変化をスムーズに通知できます。また、エラーコンテナには `role="alert"` を指定することで、エラー発生時に即座に読み上げられるようになります。
**Action:** 今後、動的バリデーションや条件付きで表示されるエラーメッセージを実装する際は、常に `aria-describedby` と `aria-live="polite"`、または `role="alert"` を併用してアクセシビリティを確保します。
