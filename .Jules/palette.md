...
## 2026-06-28 - Dynamic Error Message Accessibility
**Learning:** 動的に表示されるフォーム検証エラーや送信エラーメッセージ（例: 条件付きでレンダリングされる `{error && <div>{error}</div>}`）において、スクリーンリーダーユーザーがエラーの発生に即座に気づけるようにするためには、エラーメッセージのコンテナ要素に `role="alert"` 属性を付与することが不可欠です。これにより、要素がDOMに追加された瞬間に自動的に読み上げが行われます。
**Action:** 今後、エラーメッセージを動的に表示するUIを実装・改善する際は、必ず `role="alert"` を含め、ユーザーが視覚的にも聴覚的にも状態の変化を迅速に把握できるようにします。
